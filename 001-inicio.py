...
def create_message_frame(message, frame_size, duration, fps):
    ...
    cv2.putText(frame, message, (text_x, text_y), font, font_scale, color, font_thickness)

    return [frame for _ in range(int(duration * fps))]

def process_video(video_path, text_entries, output_path):
    ...
    try:
        ...
        while cap.isOpened():
            ...
            if (current_entry_index < len(text_entries) and
                ...
                current_message_frames = create_message_frame(message, frame_size, 5, fps)
                current_entry_index += 1

            if current_message_frames:
                out.write(current_message_frames.pop(0))
            else:
                out.write(frame)
...
def process():
    ...
    process_video(video_path, text_entries, output_path)
