...
# Memory file to store the last used directory
MEMORY_FILE = "last_directory.json"
    ...
    if os.path.exists(MEMORY_FILE):
        try:
            with open(MEMORY_FILE, 'r') as f:
                return json.load(f).get('last_directory', os.getcwd())
        except Exception:
            return os.getcwd()
    return os.getcwd()
...
def process_video(video_path, text_entries, output_path, bg_color, progress_bar):
...
def select_video():
    ...
    video_path = filedialog.askopenfilename(initialdir=initial_dir, filetypes=[("Video Files", "*.mp4")])
    if video_path:
        video_var.set(video_path)
        save_last_directory(os.path.dirname(video_path))

def select_text_file():
    initial_dir = load_last_directory()
    text_path = filedialog.askopenfilename(initialdir=initial_dir, filetypes=[("Text Files", "*.txt")])
    if text_path:
        text_var.set(text_path)
        save_last_directory(os.path.dirname(text_path))

def select_output_file():
    initial_dir = load_last_directory()
    output_path = filedialog.asksaveasfilename(initialdir=initial_dir, filetypes=[("Video Files", "*.mp4")], defaultextension=".mp4")
    if output_path:
        output_var.set(output_path)
        save_last_directory(os.path.dirname(output_path))
...
def process():
    ...
    process_video(video_path, text_entries, output_path, bg_color, progress_bar)
